  ...
  async function fetchStock() {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_ADDRESS}/GetStock/StockWithPara`,
        {
          method: "GET",
          headers: {
      ...
      if (responseData && responseData.status === 1) {
        // Dispatch success action with the UserData
        dispatch(setSuccess(responseData.UserData || []));
        dispatch(setError(null));
      } else {
        // Handle cases where status is not 1 or data is unexpected
        dispatch(
          setError(responseData.message || "Unexpected response format")
        );
        dispatch(setSuccess([]));
      }
    } catch (error) {
