...
export default function Wishlist() {
  ...
  const currentRows =
    rowsPerPage === "All"
      ? wishlistItems
      : wishlistItems?.slice(indexOfFirstRow, indexOfLastRow);
  const totalPages =
    rowsPerPage === "All" ? 1 : Math.ceil(wishlistItems?.length / rowsPerPage);
  ...
  const fetchWishlistItems = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://${process.env.REACT_APP_USER_SERVER_ADDRESS}/users/${user.success.id}/wishlist`,
        {
          headers: {
    ...
    <div className="flex w-full h-full bg-main-bg">
      ...
      <div className="flex-auto w-[85%]">
        <div className="p-4 sticky top-0 bg-main-bg z-[5] flex md:justify-between md:items-center md:flex-row flex-col gap-2">
          <div>
            <h2 className="text-4xl font-semibold text-theme-600">Wishlist</h2>
            <Link
              to={"/search"}
